...
export interface PostgresSyncRepositoryOptions {
...
export class PostgresSyncRepository implements SyncRepository {
  ...
  constructor(connectionString: string, blobStore?: OpaqueBlobStore, options: PostgresSyncRepositoryOptions = {}) {
    const maxConnections = options.maxConnections ?? 12;
    if (!Number.isInteger(maxConnections) || maxConnections < 1 || maxConnections > 12) throw new Error("Invalid PostgreSQL connection limit");
    this.#pool = new Pool({
      connectionString,
      max: maxConnections,
      idleTimeoutMillis: 30_000,
      statement_timeout: 10_000,
      application_name: options.applicationName ?? "locus-sync-service",
    });
  ...
  async bootstrap(tokenHash: string, device: AuthenticatedDevice): Promise<void> {
    await this.#pool.query(`
      INSERT INTO locus_sync.accounts(id, key_version) VALUES ($1, 1)
      ON CONFLICT (id) DO UPDATE SET key_version=GREATEST(accounts.key_version, 1)
    `, [device.accountId]);
    await this.#pool.query(`
      INSERT INTO locus_sync.devices(id, account_id, name, public_key, token_hash, key_version)
  ...
  async authenticate(tokenHash: string): Promise<AuthenticatedDevice | undefined> {
    const result = await this.#pool.query<{ account_id: string; id: string }>(
      "UPDATE locus_sync.devices SET last_seen_at=now() WHERE token_hash=$1 AND revoked_at IS NULL RETURNING account_id, id",
      [tokenHash],
    );
  ...
  async push(device: AuthenticatedDevice, keyVersion: number, records: OpaqueSyncRecord[]): Promise<{ cursor: number; accepted: number }> {
    ...
    try {
      ...
      const account = await client.query<{ key_version: number }>("SELECT key_version FROM locus_sync.accounts WHERE id=$1 FOR SHARE", [device.accountId]);
      ...
      for (const [index, record] of records.entries()) {
        ...
        const previous = await client.query<{ object_key: string | null }>(`
          SELECT object_key FROM locus_sync.sync_records WHERE account_id=$1 AND collection=$2 AND record_id=$3
        `, [record.accountId, record.collection, record.recordId]);
        const result = await client.query<{ cursor: string }>(`
          INSERT INTO locus_sync.sync_records(account_id, collection, record_id, device_id, clock, nonce, ciphertext, object_key, size, tombstone, version)
          ...
          ON CONFLICT (account_id, collection, record_id) DO UPDATE SET
            ...
            cursor = nextval('locus_sync.sync_cursor'),
            updated_at = now()
  ...
  async pull(accountId: string, cursor: number, limit: number) {
    ...
    try {
      ...
      FROM locus_sync.sync_records WHERE account_id = $1 AND cursor > $2 ORDER BY cursor ASC LIMIT $3 FOR SHARE
  ...
  async createEnrollment(enrollment: Enrollment): Promise<void> {
    await this.#pool.query(`
      INSERT INTO locus_sync.device_enrollments(id, device_id, device_name, public_key, code_hash, expires_at)
  ...
  async enrollmentDetails(enrollmentId: string, codeHash: string) {
    const result = await this.#pool.query<{
      ...
      SELECT device_id, device_name, public_key, expires_at FROM locus_sync.device_enrollments
  ...
  async approveEnrollment(accountId: string, enrollmentId: string, codeHash: string, wrappedAccountKey: string, deviceToken: string, tokenHash: string): Promise<void> {
    ...
    try {
      await client.query("BEGIN");
      const account = await client.query<{ key_version: number }>("SELECT key_version FROM locus_sync.accounts WHERE id=$1 FOR UPDATE", [accountId]);
      ...
      const enrollment = await client.query<{ device_id: string; device_name: string; public_key: string }>(`
        UPDATE locus_sync.device_enrollments SET account_id=$1, wrapped_account_key=$2, device_token=$3, token_hash=$4
      ...
      const inserted = await client.query(`
        INSERT INTO locus_sync.devices(id, account_id, name, public_key, token_hash, wrapped_account_key, key_version)
  ...
  async takeEnrollment(enrollmentId: string, codeHash: string) {
    const result = await this.#pool.query<{
      ...
      UPDATE locus_sync.device_enrollments SET claimed_at=now()
      WHERE id=$1 AND code_hash=$2 AND expires_at > now() AND claimed_at IS NULL
      ...
      RETURNING account_id, device_id, wrapped_account_key, device_token,
        (SELECT key_version FROM locus_sync.accounts WHERE id=device_enrollments.account_id) AS key_version
    `, [enrollmentId, codeHash]);
    const row = result.rows[0];
    return row ? {
      accountId: row.account_id,
  ...
  async createPasskeyCeremony(ceremony: PasskeyCeremony): Promise<void> {
    await this.#pool.query(`
      INSERT INTO locus_sync.passkey_ceremonies(
  ...
  async passkeyCeremony(id: string): Promise<PasskeyCeremony | undefined> {
    const result = await this.#pool.query<{
      ...
      SELECT id, kind, account_id, user_id, display_name, challenge, options_json,
             device_id, device_name, device_public_key, expires_at
      FROM locus_sync.passkey_ceremonies WHERE id=$1 AND expires_at > now()
    `, [id]);
    const row = result.rows[0];
    return row ? {
      id: row.id,
  ...
  async consumePasskeyCeremony(id: string, kind: PasskeyCeremony["kind"]): Promise<PasskeyCeremony | undefined> {
    const result = await this.#pool.query<{
      ...
      DELETE FROM locus_sync.passkey_ceremonies
  ...
  async createAccountWithPasskey(accountId: string, passkey: StoredPasskey, device: RegisteredDevice): Promise<void> {
    ...
    try {
      await client.query("BEGIN");
      await client.query("INSERT INTO locus_sync.accounts(id) VALUES ($1)", [accountId]);
      await client.query(`
        INSERT INTO locus_sync.passkeys(credential_id, account_id, user_id, public_key, counter, device_type, backed_up, transports)
      ...
      await client.query(`
        INSERT INTO locus_sync.devices(id, account_id, name, public_key, token_hash, wrapped_account_key, key_version, created_at, last_seen_at)
  ...
  async passkey(credentialId: string): Promise<StoredPasskey | undefined> {
    const result = await this.#pool.query<{
      ...
      FROM locus_sync.passkeys WHERE credential_id=$1
    `, [credentialId]);
    const row = result.rows[0];
    return row ? {
  ...
  async authenticateWithPasskey(credentialId: string, counter: number, device: RegisteredDevice): Promise<void> {
    ...
    try {
      ...
      const updated = await client.query(`
        UPDATE locus_sync.passkeys SET counter=$1, last_used_at=now()
  ...
  async createPasskeyClaim(claim: PasskeyClaim): Promise<void> {
    await this.#pool.query(`
      INSERT INTO locus_sync.passkey_claims(id, code_hash, account_id, device_id, device_token, expires_at)
      VALUES ($1,$2,$3,$4,$5,to_timestamp($6 / 1000.0))
  ...
  async cleanupExpired(now: number): Promise<void> {
    ...
    await this.#pool.query("DELETE FROM locus_sync.device_enrollments WHERE expires_at < $1", [timestamp]);
    await this.#pool.query("DELETE FROM locus_sync.passkey_ceremonies WHERE expires_at < $1", [timestamp]);
    await this.#pool.query("DELETE FROM locus_sync.passkey_claims WHERE expires_at < $1", [timestamp]);
    await this.#pool.query("DELETE FROM locus_sync.sync_records WHERE tombstone=true AND updated_at < $1", [new Date(now - 90 * 24 * 60 * 60 * 1_000)]);
  }
  ...
  async takePasskeyClaim(id: string, codeHash: string) {
    const result = await this.#pool.query<{ account_id: string; device_id: string; device_token: string }>(`
      DELETE FROM locus_sync.passkey_claims
  ...
  async listDevices(accountId: string): Promise<SyncDevice[]> {
    const result = await this.#pool.query<{
      ...
      FROM locus_sync.devices WHERE account_id=$1 AND revoked_at IS NULL
  ...
  async accountKeyState(accountId: string, deviceId: string): Promise<{ version: number; wrappedAccountKey?: string }> {
    const result = await this.#pool.query<{ key_version: number; wrapped_account_key: string | null; device_key_version: number }>(`
      ...
      FROM locus_sync.accounts JOIN locus_sync.devices ON devices.account_id=accounts.id
  ...
  async initializeAccountKey(accountId: string, expectedVersion: number, version: number, wraps: AccountKeyWrap[]): Promise<void> {
    ...
    try {
      ...
      for (const wrap of wraps) await client.query(`
        UPDATE locus_sync.devices SET wrapped_account_key=$1, key_version=$2
        WHERE account_id=$3 AND id=$4 AND revoked_at IS NULL
      `, [wrap.wrappedAccountKey, version, accountId, wrap.deviceId]);
      await client.query("UPDATE locus_sync.accounts SET key_version=$1 WHERE id=$2", [version, accountId]);
      await client.query("COMMIT");
    } catch (error) {
      await client.query("ROLLBACK");
      throw error;
  ...
  async setDeviceWrappedKey(accountId: string, deviceId: string, version: number, wrappedAccountKey: string): Promise<void> {
    const result = await this.#pool.query(`
      UPDATE locus_sync.devices SET wrapped_account_key=$1, key_version=$2
      WHERE account_id=$3 AND id=$4 AND revoked_at IS NULL
        AND $2=(SELECT key_version FROM locus_sync.accounts WHERE id=$3)
  ...
  async rotateAccountKey(device: AuthenticatedDevice, expectedVersion: number, version: number, wraps: AccountKeyWrap[], records: OpaqueSyncRecord[]): Promise<{ cursor: number }> {
    ...
    try {
      await client.query("BEGIN");
      const account = await client.query<{ key_version: number }>("SELECT key_version FROM locus_sync.accounts WHERE id=$1 FOR UPDATE", [device.accountId]);
      if (account.rows[0]?.key_version !== expectedVersion || version !== expectedVersion + 1) throw new Error("Sync account key version changed");
      ...
      const existing = await client.query<{ collection: string; record_id: string; object_key: string | null }>(
        "SELECT collection, record_id, object_key FROM locus_sync.sync_records WHERE account_id=$1 FOR UPDATE",
        [device.accountId],
      );
      ...
      await client.query("DELETE FROM locus_sync.sync_records WHERE account_id=$1", [device.accountId]);
      let cursor = 0;
      for (const [index, record] of records.entries()) {
        const payload = staged[index]!;
      ...
      await client.query("UPDATE locus_sync.accounts SET key_version=$1 WHERE id=$2", [version, device.accountId]);
      await client.query("COMMIT");
  ...
  async revokeDevice(accountId: string, deviceId: string): Promise<void> {
    await this.#pool.query("UPDATE locus_sync.devices SET revoked_at=now() WHERE account_id=$1 AND id=$2", [accountId, deviceId]);
  }
  ...
  async readiness(): Promise<void> {
    const result = await this.#pool.query<{ tables: string }>(`
      SELECT count(*)::text AS tables
      FROM pg_catalog.pg_class
      JOIN pg_catalog.pg_namespace ON pg_namespace.oid = pg_class.relnamespace
      WHERE pg_namespace.nspname = 'locus_sync' AND pg_class.relkind = 'r'
    `);
    if (Number(result.rows[0]?.tables ?? 0) !== 7) throw new Error("Locus sync database schema is unavailable");
  }

  async referencedObjectKeys(keys: readonly string[]): Promise<Set<string>> {
    if (keys.length > 1_000) throw new Error("Too many opaque object keys in one reconciliation batch");
    if (!keys.length) return new Set();
    const result = await this.#pool.query<{ object_key: string }>(`
      SELECT object_key
      FROM locus_sync.sync_records
  ...
  async #deleteRecordsWithAccountLock(accountId: string, deleteAccount: boolean): Promise<Array<string | null>> {
    ...
    try {
      await client.query("BEGIN");
      await client.query("SELECT id FROM locus_sync.accounts WHERE id=$1 FOR UPDATE", [accountId]);
      const existing = await client.query<{ object_key: string | null }>("SELECT object_key FROM locus_sync.sync_records WHERE account_id=$1", [accountId]);
      if (deleteAccount) {
        await client.query("UPDATE locus_sync.accounts SET deleted_at=now() WHERE id=$1", [accountId]);
        await client.query("DELETE FROM locus_sync.accounts WHERE id=$1", [accountId]);
      } else {
        await client.query("DELETE FROM locus_sync.sync_records WHERE account_id=$1", [accountId]);
      }
      await client.query("COMMIT");
...
async function currentCursor(client: PoolClient): Promise<number> {
  const result = await client.query<{ cursor: string }>("SELECT last_value AS cursor FROM locus_sync.sync_cursor");
  return Number(result.rows[0]?.cursor ?? 0);
}

async function activeDeviceIds(client: PoolClient, accountId: string): Promise<string[]> {
  const result = await client.query<{ id: string }>("SELECT id FROM locus_sync.devices WHERE account_id=$1 AND revoked_at IS NULL ORDER BY id FOR UPDATE", [accountId]);
